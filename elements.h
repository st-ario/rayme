...
struct hit_record
{
  ...
  hit_record( point p
            ...
            , std::shared_ptr<material> ptr_mat
            , normed_vec3 n
            ) : p{p}, t{at}, front_face{front_face}, ptr_mat{ptr_mat}, normal{n} {}
};

class element
{
  public:
    virtual std::optional<hit_record> hit(const ray& r, float t_max) const = 0;
};
  ...
  public:
    ...
    virtual std::optional<hit_record> hit(const ray& r, float t_max) const override
    {
        ...
        for (const auto& object : objects) {
            auto partial_record = object->hit(r, closest_so_far);
            if (partial_record) {
...
class sphere : public element
{
  public:
    ...
    std::optional<hit_record> hit(const ray& r, float t_max) const override
    {
